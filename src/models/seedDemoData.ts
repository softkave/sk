...
export default function seedDemoData({ name }: { name?: string } = {}) {
  ...
  const org_CanFactory = seedBlock(user, {
    ...
    type: BlockType.Organization,
    description: "Simple and efficient can factory. We strive to be the best.",
  });
  ...
  const board_Softkave_Marketing = seedBlock(user, {
    ...
    boardLabels: [
      ...
      },
      {
        ...
        description: "For efforts to gain traction by paid advertisements.",
        color: randomColor(),
      },
    ],
    boardResolutions: [
  ...
  const task_Softkave_Engineering_3 = seedBlock(user, {
    ...
    taskResolution: board_Softkave_Engineering.boardResolutions![0].customId,
  });
