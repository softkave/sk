...
import { default as AppMessage } from "../PageMessage";
import Scrollbar, { ScrollbarMethods } from "../utils/Scrollbar";
...
const ChatList: React.FC<IChatListProps> = (props) => {
  ...
  if (chats.length === 0) {
    return <AppMessage message="Write a message and press the enter button to send" />;
  }
  ...
  return (
    ...
    >
      {chats.map((chat, i) => {
        const sender = chat.createdBy === user.customId ? user : recipient;
        const chatRender = (
          <div
            key={i}
            style={{
          ...
          >
            <Chat
              chat={chat}
              sender={sender}
              hideAvatar={hideAvatarCheck[chat.createdBy]}
              isUserSender={sender.customId === user.customId}
            />
          </div>
        );

        hideAvatarCheck = { [chat.createdBy]: true };
        return chatRender;
      })}
    </Scrollbar>
  );
