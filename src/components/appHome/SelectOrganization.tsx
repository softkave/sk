...
import SpaceOut, { ISpaceOutContent } from "../utils/SpaceOut";
...
const SelectOrganization: React.FC<ISelectOrganizationProps> = (props) => {
  ...
  const organization = useSelector<IAppState, IAppWorkspace>((state) =>
  ...
  const spaceOutContent: ISpaceOutContent[] = React.useMemo(() => {
    const content: ISpaceOutContent[] = [
      {
        node: <BlockThumbnail key={organization.customId} block={organization} showName />,
        style: { flex: 1 },
      },
      {
        node: (
