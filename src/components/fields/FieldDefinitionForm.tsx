...
import { feedbackFieldsList, IFeedbackField, IFieldInput } from "../../models/fields/fields";
...
const FieldDefinitionForm: React.FC<IFieldDefinitionFormProps> = (props) => {
  ...
  const getNameExistsError = (name: string) => {
    if (name && name.length > 0) {
      ...
      const found = existingFields.find((field) => field.name?.toLowerCase() === name);
  ...
  const getProgrammaticIdExistsError = (pid: string) => {
    if (pid && pid.length > 0) {
      pid = pid.toLowerCase();
      const found = existingFields.find((field) => field.programmaticId?.toLowerCase() === pid);
  ...
  const renderNameInput = () => {
    ...
    const nameError = formik.errors.name || getNameExistsError(formik.values.name || "");
    ...
    return (
      <Form.Item
        ...
        labelCol={{ span: 24 }}
        wrapperCol={{ span: 24 }}
        help={formik.touched.name && <FormFieldError error={nameError} />}
        style={{ width: "100%" }}
      >
        <InputWithControls
          hideControls
  ...
  const renderProgrammaticIdInput = () => {
    ...
    return (
      <Form.Item
        ...
        labelCol={{ span: 24 }}
        wrapperCol={{ span: 24 }}
        help={formik.touched.programmaticId && <FormFieldError error={idError} />}
        style={{ width: "100%" }}
      >
        <InputWithControls
          hideControls
  ...
  const renderDescInput = () => {
    return (
      <Form.Item
        ...
        wrapperCol={{ span: 24 }}
        help={
          formik.touched.description && <FormFieldError>{formik.errors.description}</FormFieldError>
        }
      >
        <InputWithControls
          hideControls
  ...
  const renderTypeInput = () => {
    return (
      <Form.Item
        ...
        labelCol={{ span: 24 }}
        wrapperCol={{ span: 24 }}
        help={formik.touched.type && <FormFieldError>{formik.errors.type}</FormFieldError>}
      >
        <Select
      ...
      <form onSubmit={handleSubmit} className={formClassname}>
        <div style={formContentWrapperStyle}>
          <div style={formInputContentWrapperStyle}>
            ...
            {errors.error && <FormFieldError error={errors.error} />}
            {renderNameInput()}
            {renderDescInput()}
