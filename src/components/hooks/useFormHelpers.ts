...
export interface IUseFormHelpersFormikHelpers<T extends object> {
  ...
  moveInArrayField: (field: string, srcIndex: number, destIndex: number) => void;
  revertChanges: (field?: keyof T | string) => void;
}

export interface IUseFormHelpersFormikChangedFieldsHelpers<T extends object = any> {
...
const reworkReverted = debounce(
  (changedFields: IUseObjectHookResult<any>, initialValues: any, values: any) => {
...
const useFormHelpers = <T extends object>(
  ...
  const formikChangedFieldsHelpers = React.useMemo(
    ...
    [diffChanges, hasChanges, clearAll, addChangedField, removeField, pushFields]
  );
