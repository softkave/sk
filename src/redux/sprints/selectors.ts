...
function getSprintsFromIndex(state: IAppState, boardId: string, index: number): ISprint[] {
  const sprints: ISprint[] = [];
...
function getNextSprintAfterIndex(
  ...
  for (const id in state.sprints) {
    ...
    if (sprint.boardId === boardId && sprint.sprintIndex === index && sprint.nextSprintId) {
...
export default class SprintSelectors {
  static countBoardSprints = countBoardSprints;
  static getBoardSprints = getBoardSprints;
  static getSprint = getSprint;
  static getSprintsFromIndex = getSprintsFromIndex;
  static getNextSprintAfterIndex = getNextSprintAfterIndex;
}
