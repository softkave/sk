...
>("op/block/deleteBlock", async (arg, thunkAPI) => {
  ...
  thunkAPI.dispatch(dispatchOperationStarted(opId, OperationType.DeleteBlock));
  ...
  try {
    ...
    thunkAPI.dispatch(
      dispatchOperationCompleted(opId, OperationType.DeleteBlock)
    );
  } catch (error) {
    thunkAPI.dispatch(
      dispatchOperationError(opId, OperationType.DeleteBlock, error)
    );
  }
