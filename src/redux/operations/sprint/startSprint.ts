...
>("op/sprint/startSprint", async (arg, thunkAPI) => {
  ...
  thunkAPI.dispatch(
    dispatchOperationStarted(opId, OperationType.StartSprint, arg.sprintId)
  );
  ...
  try {
    ...
    thunkAPI.dispatch(
      dispatchOperationCompleted(opId, OperationType.StartSprint, arg.sprintId)
    );
  } catch (error) {
    thunkAPI.dispatch(
      dispatchOperationError(
        opId,
        OperationType.StartSprint,
        error,
        arg.sprintId
      )
    );
