...
export const removeCollaboratorOpAction = makeAsyncOp02NoPersist(
  ...
  async (arg: IRemoveCollaboratorEndpointParams, thunkAPI, extras) => {
    ...
    completeRemoveCollaborator(thunkAPI, arg.organizationId, arg.collaboratorId);
  }
);
...
export function completeRemoveCollaborator(
  ...
  let organization = OrganizationSelectors.getOne(thunkAPI.getState(), organizationId);
  if (!organization) {
    return;
  }
  ...
  organization.collaboratorIds.splice(organization.collaboratorIds.indexOf(collaboratorId), 1);
  thunkAPI.dispatch(
  ...
  const organizations = OrganizationSelectors.filter(
    thunkAPI.getState(),
    (item) => item.collaboratorIds.includes(collaboratorId) && item.customId !== organizationId
  );
