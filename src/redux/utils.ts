...
export function toActionAdd<T>(item: T, key: keyof T): IActionAdd<T> {
  return { id: get(item, key), data: item };
}

export function toActionAddList<T>(items: T[], key: keyof T): IActionAdd<T>[] {
...
export function getSelectors<T>(
  ...
  return class {
    ...
    public static assertGetOne = assertGetOne;
