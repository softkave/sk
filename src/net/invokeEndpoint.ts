...
export async function invokeEndpoint<T extends IEndpointResultBase>(
  ...
  try {
    ...
    const fullPath =
      defaultTo(basePath, apiType === "REST" ? getRESTServerAddr() : getGraphQLServerAddr()) + path;
...
export async function invokeEndpointWithAuth<T extends IEndpointResultBase>(
  ...
  const requestToken = props.token || getTokenFromStore() || UserSessionStorageFns.getUserToken();
