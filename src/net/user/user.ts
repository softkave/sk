...
import { IPersistedClient, IUser } from "../../models/user/types";
...
const newAnonymousUserPath = `${userBasePath}/newAnonymousUser`;
...
export type IUpdateUserEndpointErrors = GetEndpointResultError<IUpdateUserAPIProps>;
...
async function updateUser(props: IUpdateUserAPIProps): Promise<IUserLoginResult> {
...
async function changePassword(props: IChangePasswordAPIProps): Promise<IUserLoginResult> {
  return invokeEndpointWithAuth<IUserLoginResult>({
...
async function forgotPassword(props: IForgotPasswordAPIProps): Promise<IEndpointResultBase> {
...
async function userExists(props: IUserExistsAPIParams): Promise<IUserExistsAPIResult> {
...
async function changePasswordWithToken(
  ...
  return invokeEndpointWithAuth<IUserLoginResult>({
    ...
    apiType: "REST",
    data: changePasswordWithTokenYupSchema.validateSync(props, endpointYupOptions),
    token: props.token,
  });
}

export interface IUpdateClientEndpointParams {
  data: Omit<IPersistedClient, "customId">;
}
...
export type IUpdateClientEndpointErrors = GetEndpointResultError<IUpdateClientEndpointParams>;
...
async function getUserData(props: IGetUserDataAPIParams): Promise<IUserLoginResult> {
  return invokeEndpointWithAuth<IUserLoginResult>({
...
async function newAnonymousUser(): Promise<IUserLoginResult> {
  return invokeEndpoint<IUserLoginResult>({
    path: newAnonymousUserPath,
    apiType: "REST",
  });
}
...
export default class UserAPI {
  ...
  static newAnonymousUser = newAnonymousUser;
}
