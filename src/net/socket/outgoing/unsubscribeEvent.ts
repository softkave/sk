...
import { IOutgoingSubscribePacket, OutgoingSocketEvents } from "../outgoingEventTypes";
import { getSocketRoomInfo, getSocketRoomName } from "../roomNameHelpers";
import SocketAPI from "../socket";
import { roomItemsYupSchema } from "./subscribeEvent";

export default async function unsubcribeEvent(items?: ClientSubscribedResources) {
  ...
  items = roomItemsYupSchema.validateSync(items, endpointYupOptions) as ClientSubscribedResources;
  ...
  const promise = SocketAPI.promisifiedEmit(OutgoingSocketEvents.Unsubscribe, data);
