...
function handleCreateChat(packet: IIncomingResourceUpdatePacket<IChat>) {
  ...
  const isUserInRoom = isUserViewingChatRoom(room.workspaceId, room.recipientId);
  const isAppHidden = KeyValueSelectors.getByKey<boolean>(
    store.getState(),
    KeyValueKeys.IsAppHidden
  );
  const markAsUnread = (!isUserInRoom || isAppHidden) && chat.createdBy !== user.customId;
...
export function handleIncomingChatEvent(packet: IIncomingResourceUpdatePacket<IChat>) {
